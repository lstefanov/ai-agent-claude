...
<div class="max-w-7xl mx-auto" x-data="planAb()">
    <div class="flex items-center justify-between mb-6 gap-4 flex-wrap">
        <div>
            ...
            <h1 class="text-2xl font-bold text-ink">A/B сравнение на плана</h1>
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('flows.builder', $flow) }}" class="text-sm text-muted hover:text-primary">← Към builder-а</a>
            <button @click="start()" :disabled="anyRunning()"
                    class="bg-primary hover:bg-primary-hover disabled:opacity-50 text-white text-sm font-medium px-4 py-2 rounded-lg transition">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        <template x-for="label in labels" :key="label">
            <div class="bg-surface rounded-xl border border-line shadow-sm overflow-hidden flex flex-col"
                 :class="!isAvailable(label) ? 'opacity-60' : ''">
                <div class="px-4 py-3.5 border-b border-line flex items-start justify-between gap-2">
                    <div class="min-w-0 flex-1">
                        <div class="font-semibold text-ink truncate" x-text="meta[label].title"></div>
                        <template x-if="label === 'hybrid'">
                            <div class="text-xs text-subtle truncate" x-text="result(label).model || meta[label].model"></div>
                        </template>
                        <template x-if="label !== 'hybrid'">
                            <div class="mt-1">
                                ...
                                <select x-model="cardModel[label]" :disabled="running[label]"
                                        class="w-full border border-line rounded-lg px-2 py-1 text-xs bg-surface text-ink focus:outline-none focus:ring-2 focus:ring-primary/40 disabled:opacity-50">
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        <template x-if="result(label).status === 'running'">
                            <span class="text-xs text-primary animate-pulse">планира…</span>
                        </template>
                        ...
                        <template x-if="result(label).status === 'skipped'">
                            <span class="text-xs text-subtle">пропуснат</span>
                        </template>
                        <button x-show="label === 'hybrid'" type="button" @click="hybridCfgOpen = true"
                                class="text-xs bg-surface border border-line hover:border-violet-500 text-muted hover:text-violet-700 font-medium px-2.5 py-1 rounded-lg transition"
                                title="Избери провайдър/модел за всяка фаза">
                            ⚙ Конфигурирай
                        </button>
                        <button @click="start(label)"
                                :disabled="running[label] || !isAvailable(label)"
                                class="text-xs bg-surface border border-primary hover:border-primary text-primary disabled:opacity-40 disabled:cursor-not-allowed font-medium px-2.5 py-1 rounded-lg transition"
                ...
                <template x-if="result(label).status === 'completed'">
                    <div class="px-4 py-2 bg-surface-subtle/70 border-b border-line text-xs text-muted tabular-nums flex items-center gap-3">
                        <span class="font-semibold text-ink" x-text="result(label).agents.length + ' агента'"></span>
                ...
                <div class="p-4 flex-1 flex flex-col">
                    ...
                    <template x-if="result(label).status === 'completed'">
                        <div class="flex-1 flex flex-col gap-2">
                            ...
                            <div class="text-xs text-muted leading-relaxed flex-1">
                                <template x-for="(agent, i) in result(label).agents.slice(0, 4)" :key="label + '-a' + i">
                                    <div class="truncate">
                                        <span class="text-subtle" x-text="(i + 1) + '.'"></span>
                                        <span class="text-ink font-medium" x-text="agent.name"></span>
                                        <span class="text-subtle" x-text="'· ' + agent.type"></span>
                                    </div>
                                </template>
                                <div x-show="result(label).agents.length > 4" class="text-subtle mt-0.5"
                                     x-text="'… и още ' + (result(label).agents.length - 4)"></div>
                            </div>

                            <button @click="dag.show(result(label).agents)" type="button"
                                    class="w-full bg-surface border border-primary hover:border-primary text-primary text-sm font-medium px-4 py-2 rounded-lg transition">
                    ...
                    <template x-if="!result(label).status && !running[label]">
                        <div class="text-sm text-subtle text-center py-10">
    ...
    <template x-if="completedLabels().length >= 2">
        <div class="mt-6 bg-surface rounded-xl border border-line shadow-sm p-5 text-sm">
            <div class="font-semibold text-ink mb-2">Разлики (типове агенти, които има само един план)</div>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <template x-for="label in completedLabels()" :key="'diff-' + label">
                    <div>
                        <span class="text-muted" x-text="meta[label].title + ':'"></span>
    ...
    <div x-show="hybridCfgOpen" x-cloak class="fixed inset-0 z-[60] flex items-center justify-center p-4"
        ...
        <div class="relative bg-surface rounded-2xl shadow-2xl w-full max-w-2xl max-h-[88vh] flex flex-col" @click.stop>
            <div class="px-6 py-4 border-b border-line shrink-0">
                <h3 class="text-lg font-bold text-ink">Хибрид — конфигурация по фази</h3>
                <p class="text-xs text-subtle mt-0.5">Различен провайдър/модел за всяка фаза на планирането.</p>
            </div>
            <div class="p-6 overflow-y-auto">
                <div class="mb-4">
                    <label class="block text-xs font-medium text-muted mb-1">Ниво на моделите за агентите</label>
                    <select x-model="level" class="w-full border border-line rounded-lg px-2 py-2 text-sm bg-surface focus:outline-none focus:ring-2 focus:ring-violet-500">
                        <option value="low">Ниско — основно Ollama, до 3 евтини cloud</option>
                        <option value="medium">Средно — евтини cloud, поне 3 на Ollama (по подразбиране)</option>
            ...
            <div class="px-6 py-4 border-t border-line flex justify-end gap-2 shrink-0">
                <button type="button" @click="hybridCfgOpen = false" class="px-3 py-2 rounded-lg border border-line text-muted hover:bg-surface-subtle text-sm">Затвори</button>
    ...
    <div x-show="savePopup.open" x-cloak class="fixed inset-0 z-[60] flex items-center justify-center p-4"
        ...
        <div class="relative bg-surface rounded-2xl shadow-2xl w-full max-w-md" @click.stop>
            <div class="px-6 py-4 border-b border-line">
                <h3 class="text-lg font-bold text-ink">Запази като шаблон</h3>
                <p class="text-xs text-subtle mt-0.5" x-text="savePopup.label ? 'План от ' + meta[savePopup.label].title : ''"></p>
            </div>
            <div class="p-6 space-y-3 text-sm">
                <div>
                    ...
                    <input type="text" x-model="savePopup.name" @keydown.enter="confirmSave()"
                           class="w-full border border-line rounded-lg px-2.5 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
                </div>
            ...
            <div class="px-6 py-4 border-t border-line flex justify-end gap-2">
                <button type="button" @click="savePopup.open = false" class="px-3 py-2 rounded-lg border border-line text-muted hover:bg-surface-subtle text-sm">Отказ</button>
