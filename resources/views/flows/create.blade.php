...
<div x-data="flowCreator()" x-init="init()">
    ...
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-display font-bold text-ink mt-2">Нов flow</h1>
        <p class="text-muted mt-1 text-sm max-w-prose">Опиши flow-а. След запис AI ще генерира агентите автоматично в Граф Редактора.</p>
    </div>
    ...
    <form action="{{ route('companies.flows.store', $company) }}" method="POST" id="flow-form">
        ...
        <div class="bg-surface rounded-xl border border-line p-6 mb-6">
            <h2 class="text-base font-display font-semibold text-ink mb-4">Основна информация</h2>
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-ink mb-1">Наименование на flow-а</label>
                ...
                <div>
                    <div class="flex items-center justify-between mb-1">
                        <label class="block text-sm font-medium text-ink">Описание на flow-а</label>
                        <button type="button"
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('companies.show', $company) }}"
               class="px-5 py-2.5 rounded-lg border border-line text-muted hover:bg-surface-subtle transition font-medium">
                Откажи
            </a>
            <button type="submit"
                    :disabled="!flowName.trim() || !flowDescription.trim()"
                    class="inline-flex items-center gap-2 px-5 py-2.5 rounded-md bg-primary hover:bg-primary-hover disabled:opacity-50 disabled:cursor-not-allowed text-primary-fg font-medium transition">
                <x-icon name="sparkles" size="4" /> Запази и генерирай агенти
            </button>
        </div>
    </form>
</div>
