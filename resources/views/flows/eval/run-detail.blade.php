...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <div class="text-sm text-subtle mb-1">
            ...
            <a href="{{ route('flows.eval.results', $flow) }}" class="hover:text-primary">Eval резултати</a>
        </div>
        <h1 class="text-2xl font-bold text-ink">{{ $evalRun->evalCase->name ?? 'Eval run' }}</h1>
        <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted mt-2">
    ...
    {{-- Criteria --}}
    <div class="bg-surface rounded-xl border border-line p-5 mb-5">
        <h2 class="font-semibold text-ink mb-3">Критерии</h2>
        <div class="space-y-3">
            @forelse($detail as $key => $res)
                @php $crit = $criteria[$key] ?? []; $score = (int) ($res['score'] ?? 0); @endphp
                <div class="border-b border-line last:border-0 pb-3 last:pb-0">
                    <div class="flex items-center justify-between gap-3">
                        <span class="font-medium text-ink">
                            {{ $icon($score) }} {{ $crit['label'] ?? $key }}
                            @if(($crit['type'] ?? '') !== 'llm_judge')<span class="text-xs text-subtle">[{{ $crit['type'] ?? 'rule' }}]</span>@endif
                        </span>
                        <span class="text-sm shrink-0">
                            <b class="{{ $scoreClass($score) }}">{{ $score }}/100</b>
                            <span class="text-subtle">· тежест {{ rtrim(rtrim(number_format((float) ($crit['weight'] ?? 1), 1), '0'), '.') }}</span>
                        </span>
                    </div>
                    @if(!empty($res['reason']))
                        <p class="text-sm text-muted mt-1">{{ $res['reason'] }}</p>
                    @endif
                </div>
            @empty
                <p class="text-sm text-subtle">Няма оценени критерии.</p>
            @endforelse
        </div>
    </div>
    ...
    {{-- Final output --}}
    <div class="bg-surface rounded-xl border border-line p-5 mb-5">
        <h2 class="font-semibold text-ink mb-3">Финален изход</h2>
        <div class="text-sm text-ink whitespace-pre-wrap max-h-96 overflow-auto bg-surface-subtle rounded-lg p-4">{{ $evalRun->final_output ?: '(празно)' }}</div>
    </div>

    {{-- Node runs --}}
    @if($evalRun->flowRun && $evalRun->flowRun->nodeRuns->count())
    <div class="bg-surface rounded-xl border border-line p-5">
        <h2 class="font-semibold text-ink mb-3">Node runs</h2>
        <table class="w-full text-sm">
            <thead><tr class="text-subtle text-xs border-b border-line">
            ...
            <tbody>
                @foreach($evalRun->flowRun->nodeRuns as $nr)
                    <tr class="border-b border-line last:border-0">
                        <td class="py-2 text-ink">{{ $nr->node_key }}</td>
                        <td class="py-2 text-muted font-mono text-xs">{{ $nr->model_used ?: '—' }}</td>
                        <td class="py-2 text-right">{{ $nr->qa_score !== null ? $nr->qa_score : '—' }}</td>
                        <td class="py-2 text-right text-muted">{{ $fmtCost($nr->cost_usd) }}</td>
                        <td class="py-2 text-right text-xs text-subtle">{{ $nr->status }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
