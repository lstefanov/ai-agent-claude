...
Codex / Cursor / Antigravity влизат като cross-vendor проверки (локален Ollama остава евтин worker през `claude-local.sh`). Това
...
scripts/
  ai-review.sh         # cross-vendor wrapper (codex/cursor/agy)
...
# Локален Ollama като евтин worker (по избор, за scripts/claude-local.sh)
ollama pull qwen3-coder:30b     # или ползвай съществуващия qwen3:14b от .env
# хостът се чете от OLLAMA_URL в .env (напр. http://192.168.0.19:11434)
```
...
- `scripts/ai-review.sh` ползва `codex` / `cursor-agent` / `agy`, които **не докосват**
...
| cross-vendor ревю | Codex / Cursor / Antigravity | различен vendor лови други грешки |
...
- **Ollama (worker):** локален, **нулев** marginal разход — бутни евтината черна работа натам
  през `scripts/claude-local.sh` (не е част от `ai-review.sh`).
...
- Удариш ли седмичния лимит — изнеси повече ревюта на Codex/Cursor/Antigravity.
