...
документите, и го логвай в PROGRESS. Планът е каноничен, но ако се разминава с реалния код —
следвай реалния код, адаптирай се и отбележи разминаването в PROGRESS (не спирай заради това).
Спазвай CLAUDE.md (без тестове, без legacy, само services, pint), DESIGN.md/токените (никакъв
hardcoded hex) и активирай frontend-design skill за UI.

Преди старт увери се, че Redis върви и че е пуснат `composer dev` (за Horizon). Виж docs/AI-ORG-PROGRESS.md:
ако има отметнати фази — продължи от първата неотметната; иначе започни от Фаза 0.
```
