...
worker-а липсва (`worker_alive=false` в poll endpoint-а). Heartbeat-ът
(`queue.heartbeat.flows`, TTL 180s) се опреснява от **supervisor loop-а** на
Horizon (`SupervisorLooped` event в `AppServiceProvider`) — supervisor-ът се
върти непрекъснато, така че ключът остава жив и докато единственият flows
worker е зает с дълъг node (crawl/LLM/изображение). Worker-ските `Looping`/
`JobProcessing` event-и го опресняват допълнително. Банерът значи „няма жив
Horizon supervisor за опашка flows", НЕ „worker-ът е зает".
