...
return [
    ...
    // Жизнен цикъл на членовете — предпазни прагове за АВТОНОМНО съкращение (§fire-guard).
    // Автономно предложение за съкращение е допустимо само ако членът е минал стажа И е имал
    // реален шанс да покаже резултат: иначе нов, още непроверен човек (0 runs, „няма данни" qa)
    // изглежда неразличим от хроничен слаб служител. Гейтът важи САМО за автономните предложения —
    // ръчното човешко съкращение през org графа е явен override и не минава оттук.
    'lifecycle' => [
        'fire_grace_days' => (int) env('ORG_FIRE_GRACE_DAYS', 14),       // под този стаж (дни) → изпитателен, без съкращение
        'fire_min_runs' => (int) env('ORG_FIRE_MIN_RUNS', 3),            // под толкова завършени изпълнения → няма достатъчно доказателства
        'fire_low_qa_threshold' => (float) env('ORG_FIRE_LOW_QA', 40),   // qa под този праг = „траен нисък резултат" (за промпта)
    ],
