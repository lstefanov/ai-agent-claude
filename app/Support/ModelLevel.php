 ...
 * Bulgarian prose stays on local BgGPT except on Ultra/GOD (bgStaysLocal).
 *
 * GOD is the ceiling: every non-vision agent runs on the most expensive
 * flagship (gpt-4o / claude-sonnet-4-6, via PaidModel::pinTop), task-aware
 * split between OpenAI and Anthropic with no caps.
 */
enum ModelLevel: string
{
    ...
    /** High/Ultra/GOD: every non-exempt agent gets a cloud pin. */
    public function forcesCloud(): bool
    {
        return $this === self::High || $this === self::Ultra || $this === self::God;
    }

    /** Bulgarian prose stays on local BgGPT — everywhere except Ultra/GOD. */
    public function bgStaysLocal(): bool
    {
        return $this !== self::Ultra && $this !== self::God;
    }

    /** GOD pins the most expensive flagship models (PaidModel::pinTop). */
    public function usesFlagship(): bool
    {
        return $this === self::God;
    }
...
TXT,
            self::God => <<<'TXT'
provider (ниво на разходите: GOD — максимум): ВСЕКИ агент е "openai" ИЛИ "anthropic"
   на най-скъпите флагмански модели, БЕЗ лимит за броя на нито един от двата,
   ВКЛЮЧИТЕЛНО агентите, които пишат български текст. Препоръчай "anthropic" за
   разсъждение/синтез/креативност и "openai" за стриктен JSON/извличане — кодът
   прави финалния избор според задачата. Само vision/обработка на изображения
   остава "ollama".
TXT,
        };
    }
