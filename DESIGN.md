...
**Operational AI organization.** Светла, професионална, плътна работна среда в **два регистъра**. Характерът идва от човешки профили, роли, умения и отговорности, не от игрова терминология.
...
- **Профилни повърхности** (Екип, Карта на уменията, Профил на служителя, чат със служители) носят **човешка идентичност**: портрети, функционални цветове по роля/домейн, ниво/кредити, напредък — премиум и четимо, НЕ неоново/детинско.

Профилният слой е идентичност върху сериозен пулт: числата и статусите остават безмилостно ясни (tabular, иконка+текст+цвят, WCAG AA). Виж и `PRODUCT.md` (брандови стълбове).
...
- **Функционален / категориен слой (за служители/отдели/умения):** набор отделни тонове, всеки = **функция/домейн** (маркетинг, продажби, данни…), стабилно mapping-нат, не декорация. Токени `char-{purple|teal|coral|blue|amber|pink|green}`, всеки с три стопа: `-soft` (≈50 фон), база (рингове/стат-барове), `-strong` (≈800 текст, ≥4.5:1 на soft). Покриват аватар-рингове, стат-барове, възли в картата на уменията, тагове на профил. Цветът = функция, не `member.id % 7`.
- **Акценти:** `star` = gold `#c8881a` за нивото; **кредитите** ползват azure метъра (системен ресурс, не нов цвят).

Стратегия (еволюирана): **оперативните повърхности остават restrained** (azure ≤ ~10% от повърхността); **профилните повърхности ползват функционалния палет като идентичност** (плътни fill-ове, по един доминиращ тон на карта). Все още **без градиенти по подразбиране и без glassmorphism**; топлината идва от портрети и характер, не от наситен фон.
...
## Components — профилен слой (Phase 2)

Към единния език се добавят: `<x-avatar>` (портрет с функционален ринг + **fallback инициали** при `pending`/`failed`), `<x-stars>` (5-слот ★ gold = `ModelLevel`; индикатор наследено vs override), `<x-stat-bar>` (черта 0–100, функционален fill), профил-карта (аватар, две имена/роля/възраст, ниво, мини статове, статус, „кой кого управлява"), възел в картата на уменията (active/premium/locked състояния), `<x-credit-meter>` (баланс/прогрес, azure), карта на предложена задача (от служител) и `<x-decision-item>` (чакащо одобрение). Всички спазват иконка+текст+цвят, видим focus и reduced-motion.
